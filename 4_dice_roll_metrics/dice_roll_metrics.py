...
while True:
    if gfx.switch_pressed(SWITCH_A): 
        ...
        # Send the metric to Prometheus / Grafana Cloud.
        prometheus = PrometheusRemoteWritePayload()
        prometheus.add_data(
            "roll", { "instance": device_identifier }, number, time.time() * 1000
        )
        ...
        response = requests.post(
            ...
            headers = {
                ...
                "User-Agent": "Grafana-MicroPython-Workshop",
                "X-Prometheus-Remote-Write-Version": "1.0.0"
            },
            data = prometheus.get_payload()
        )

        # TODO deal with network error cases...
        print(response)
